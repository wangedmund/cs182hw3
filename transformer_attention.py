...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        tensorLength = len(keys.shape)
        if tensorLength == 4:
            similarity = th.matmul(queries, keys.permute(0, 1, 3, 2)) / th.sqrt(key_dim) # Compute the similarity according to the QKV formula
        else:
            similarity = th.matmul(queries, keys.permute(0, 2, 1)) / th.sqrt(key_dim)
        ...
        weights =  nn.Softmax(tensorLength - 1)(masked_similarity) # Turn the similarity into a normalized output. Remember that the last dim contains the features
